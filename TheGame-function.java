...
// TheGame.java   [TheGame-function.java]
// Contains a function placeMiddleBottom()  that repositions the paddle 
// into the middle-bottom of the screen each time we score
// answer to activity "6.5 Creating a function"
// The function is: private void placeMiddleBottom() at line 172
// It also contains the ‘updateBallCollusion’ function as defined on p.16 of "6-2_Guide_to_functions.pdf" (L177)
// by A. Andreatos, 31 march 2014. 
...
public class TheGame extends GameThread{
	...
	private void placeMiddleBottom() {
		mPaddleX = mCanvasWidth/2; // this is the desired function
	}
	...
	/*Get the present velocity (this should also be the velocity going away after the collision)*/
		float velocityOfBall = (float) Math.sqrt(mBallSpeedX*mBallSpeedX +
				...
				mBallSpeedY*mBallSpeedY);
				/*using the fraction between the original velocity and present velocity to calc the needed*/
	...
	protected void updateGame(float secondsElapsed) {
		...
		if(mMinDistanceBetweenRedBallAndBigBall >= distanceBetweenBallAndPaddle) {
			...
			//Increase score
			updateScore(1);
			// place the paddle into the middle-bottom of the screen
			// mPaddleX = mCanvasWidth/2;
			placeMiddleBottom(); // call function placeMiddleBottom(); 
